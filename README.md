...
評估指標：**NDCG@5**（指數增益，$\frac{2^{rel}-1}{\log_2(rank+2)}$，以 flat 方式對 test set 全部 query 取均值）。
...
三個 checkpoint（REF、無 FGM、V30 增強訓練）在 noisy test 上全部降至 NDCG@5 ≈ 0.307，
相對 clean test 跌幅高達 **64.8–64.9%**，且彼此差異不超過 0.0006。
...
V30 結合三個最優發現（無 R-Drop + alpha=0.12 + 噪聲增強），clean test NDCG@5=0.8749（+0.0030），
**低於單獨移除 R-Drop 的 C3（0.8787）**。

這說明消融實驗的改進不具線性可加性：
- 噪聲增強引入了 4,182 個困難樣本，對 clean test 有輕微負面影響
- C3 的超高分（+0.0068）可能部分源於該 run 的訓練隨機性，需多次 seed 驗證

**實際部署建議**：以 C3 配置（無 R-Drop，其餘與 v2.9 相同）作為下一版本起點。
