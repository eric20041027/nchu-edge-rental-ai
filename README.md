...
| 分數 | 分類 | 詳細說明 |
| :--- | :--- | :--- |
| **3** | **完美匹配 (Perfect)** | 預算、地點、房型、設施及所有特殊要求（如：可養寵、非頂加）全部精確滿足。 |
| **2** | **優良匹配 (Good)** | 大部分要求符合，僅有微小偏差（例如：預算超標 15% 以內，或缺少一項次要設施）。 |
| **1** | **部分符合 (Partial)** | 僅有單一維度符合（例如：只有地點對，但預算或房型完全不符）。 |
| **0** | **硬衝突 (Conflict)** | 存在不可妥協的衝突，如：**性別不符**、**房型不對**、**命中明確排除項**。 |

### 2. 核心排序指標
*   **NDCG @ 5 (Normalized Discounted Cumulative Gain)**:
    *   **意義**：衡量前 5 名結果的「綜合質量」。它對排在前面的完美房源 (3分) 給予極高分，若高品質房源被排到後面，分數會大幅衰減。
    *   **目標**：NDCG 越接近 1.0，代表最完美的房源越能出現在搜尋結果的首位。
*   **MRR (Mean Reciprocal Rank)**:
    *   **意義**：衡量「第一個相關結果」出現的位置。如果第一個相關房源出現在第 1 名，得分為 1.0；若在第 2 名，則為 0.5。
