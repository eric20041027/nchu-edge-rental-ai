...
4. **泛化強化**(LLM 生成多樣 query 破模板 + 真評估集 + 重訓)— ✅ 階段④已落地 main(unified 0.739 天花板結案)
5. **載入優化**(bi-encoder rbt6→rbt3 蒸餾)— ✅ 階段⑤已落地 main(57→38MB,召回零損,PR #78)
6. **規模化擴展**(CI 護欄 → 接新平台擴量 → 召回品質)— 意圖見下方〈階段⑥確認意圖〉

## 階段⑥確認意圖

> 2026-06-27 規劃時房源仍 704 筆;**2026-06-28 已擴量到 974**(租租通 665 + 591 162 + 興大 147,#81/#83)。下方為規劃當下的歷史記錄。
> 階段③管線打通時即定調「先管線、後平台」,接平台從未做。本階段補上,並先鋪工程護欄。

- **Outcome:** 把專案從「能跑的 demo」推向「能持續擴量的系統」,順序 **B→A→C**:
  - **B 工程護欄:** 加 CI(GitHub Actions)自動跑既有測試 + recall gate,PR 退步自動擋。
  - **A 接平台擴量:** 沿用階段③兩段管線,接新租屋平台 crawler,704 → 幾千筆。
  - **C 召回品質:** 擴量後的真實資料上 hard-negative mining 重訓 bi-encoder(條件式,視 A 後資料量決定深度)。
- **User:** 維護者 —— 後續每次改 crawler/重訓不再靠手動驗(B);demo 使用者 —— 房源更多更有料(A)。
- **Why now:** 五階段鋪好可擴展性與品質,但量還停在 704。護欄先行讓擴量安全,再兌現 roadmap 主目的地。
- **Success(行為判準):**
  - B:假退步 PR(改壞 recall)→ CI 紅;正常 PR → 綠。
  - A:房源數明顯上升、前端暴力 cosine 仍即時、recall gate 仍 GO。
  - C:recall gate 數字較 A 後提升,仍 GO(撞天花板則誠實記錄不硬推)。
- **Constraint:** edge-first / 零後端不變;`geo_tier`/`distance` 維持相對中興(不跨城市);
  CI 輕量(job 1 不裝 torch,缺則自動 skip distill 測試);先 B 後 A 後 C。

### 階段⑥ Out of scope

- 向量 DB / ANN(規模假設幾千~萬筆,暴力 cosine 即足夠 — 沿用既有 out of scope)
- 跨城市 / 跨地理語境
- 後端推論服務
- C 階段換更大 encoder / 離開 edge
