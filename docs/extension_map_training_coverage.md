...
相關:`eval_ce_query_expansion.py`(query 端擴展 A/B,淨中性高方差)、`ce_text_layer_decision.md`(房源端 enrichment NO-GO)。三者共同描繪 extension map 與 CE 的完整關係:**推論時前處理,不碰訓練,且融入訓練不值得。**
