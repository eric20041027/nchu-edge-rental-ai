...
4. **向量召回（bi-encoder）**：query 經 bi-encoder ONNX 編碼後，於瀏覽器端對 704 筆房源向量做暴力 cosine，取 Top-30 作為主要召回（rule-based 關鍵字召回降為 fallback）
...
| 項目 | 數值 |
|:---|:---|
| 召回模型 | bi-encoder rbt6 Dynamic INT8（`bi_encoder_dir/bi_encoder_quant.onnx`，**57.0 MB**）|
| Worker | `frontend/js/bi-encoder-worker.js`（獨立 Web Worker）|
| 輸出 | 768 維 embedding，graph 內含 mean-pool + L2-norm |
| 房源向量 | `frontend/assets/property_embeddings.json`，704 × 768 float16（~4.3 MB），已 L2-normalized |
| 召回流程 | query → bi-encoder 編碼 → mean-pool/L2-norm → 對 704 筆房源向量暴力 cosine → Top-30（與 hard-exclusion allowed set 取交集）|
| cosine 成本 | 純 JS 迴圈，704 × 768 規模極小，可忽略 |
...
> bi-encoder 尚未單獨做 per-pass latency / 載入時間 benchmark；此處僅列已驗證的檔案大小（57.0 MB）作為新增首次載入權重。
...
>
> **首次載入現為三個模型**：NER + Cross-Encoder + bi-encoder。新增的 bi-encoder（**57.0 MB** INT8）外加 `property_embeddings.json`（~4.3 MB），是疊加在原 NER+CE 之上的新首次載入權重；bi-encoder 尚未單獨 benchmark 載入時間，上表未含其數字。Service Worker 於首次載入後快取所有模型。
