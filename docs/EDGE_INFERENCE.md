...
> 現用模型為 2026-06-16 C 組富化 rbt3 student，大小由舊版 57.4 MB 降至 38.7 MB（舊模型已備份 `.PREV-20260616.onnx`）。Accuracy/F1 為原 per_channel 評估值，富化重訓後 NDCG@5 0.9351 → 0.9475。延遲欄為舊 MAX_LENGTH=64 基準，128 token 實機數字見下方「實測延遲」。
...
> 上表 Cross-Encoder 載入數字為舊 57 MB 模型的 benchmark。production 已換 C 組富化 rbt3 student（**38.7 MB**），檔案更小，首次載入應更快（尚未重測；此處保留舊實測為歷史對照）。
